...
//debug数据
uint16_t time_cali,flag_action;
...
//        CAN_cmd_chassis_reset_ID();
    ...
    if (rc.ch0 < -RC_CALI_VALUE_HOLE && rc.ch1 < -RC_CALI_VALUE_HOLE && rc.ch2 > RC_CALI_VALUE_HOLE && rc.ch3 < -RC_CALI_VALUE_HOLE && rc.sw1==2 && rc.sw2==2 && rc_action_flag == 0)
    {
        //two rockers set to  \../, hold for 2 seconds,
        //两个摇杆打成 \../,保持2s
        rc_cmd_time++;
    }
    else if (rc.ch0 > RC_CALI_VALUE_HOLE && rc.ch1 > RC_CALI_VALUE_HOLE && rc.ch2 < -RC_CALI_VALUE_HOLE && rc.ch3 > RC_CALI_VALUE_HOLE && rc.sw1==2 && rc.sw2==2 && rc_action_flag != 0)
    {
